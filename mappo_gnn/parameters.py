...
STATE_FC_INPUT_DIMS = 0
STATE_FC_OUTPUT_DIMS = 0
U_ACTION_DIMS=0
C_ACTION_DIMS=0
GNN_INPUT_DIMS=0
GNN_OUTPUT_DIMS=0
...
alg_args={'mode':MODE,'scenario_name':SCENARIO_NAME,'training_name':TRAINING_NAME,'lr_actor':LR_ACTOR,'lr_critic':LR_CRITIC,
   'num_agents':NUM_AGENTS, 'num_opp':NUM_OPP, 'u_range':U_RANGE, 'u_noise':U_NOISE, 'c_noise':C_NOISE,
   'is_adversary': IS_ADVERSARY, 'actor_dropout_p': ACTOR_DROPOUT_P, 'critic_dropout_p':CRITIC_DROPOUT_P,
   'state_fc_input_dims':STATE_FC_INPUT_DIMS,'state_fc_output_dims':STATE_FC_OUTPUT_DIMS,
   ...
   'gmt_output_dims':GMT_OUTPUT_DIMS,'fc_output_dims':FC_OUTPUT_DIMS,'batch_size':BATCH_SIZE,'gamma':GAMMA,
   'clip_coeff':CLIP_COEFF, 'num_epochs':NUM_EPOCHS, 'gae_lambda':GAE_LAMBDA, 'entropy_coeff':ENTROPY_COEF,
   'use_huber_loss':USE_HUBER_LOSS, 'huber_delta':HUBER_DELTA, 'use_clipped_value_loss':USE_CLIPPED_LOSS,
   'critic_loss_coeff':CRITIC_LOSS, 'grad_clipping':GRAD_CLIPPING, 'grad_norm_clip':GRAD_NORM_CLIP
}
