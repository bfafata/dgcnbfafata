...
from utils.dgcn import DGCNConv
...
class dgcn_block(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, att_heads = 1, mul_att_heads = 1, groups = 1, concat = True, negative_slope = 0.2, dropout = 0.0, add_self_loops = True, edge_dim = None, fill_value = 'mean', bias = True, 
                 activation_func=None, weight_initialisation=None, *args, **kwargs):
...
class mappo_dgcn_actor_model(nn.Module):
    ...
    def __init__(self, model, model_name, mode, scenario_name, training_name, learning_rate, optimizer, lr_scheduler, obs_dims, dgcn_output_dims, somu_lstm_hidden_size, somu_lstm_num_layers, somu_lstm_dropout, num_somu_lstm, 
        ...
        # final fc_blocks for actions with softmax activation function
        self.softmax_actions_layer = fc_block(input_shape = actor_fc_output_dims[-1], output_shape = softmax_actions_dims, activation_func = "softmax", dropout_p = softmax_actions_dropout_p, weight_initialisation = "default")
...
class mappo_dgcn_critic_model(nn.Module):
    ...
    def __init__(self, model, model_name, mode, scenario_name, training_name, learning_rate, optimizer, lr_scheduler, gatv2_input_dims, gatv2_output_dims, gatv2_num_heads, gatv2_dropout_p, gatv2_bool_concat, gmt_hidden_dims, 
                 gmt_output_dims, fc_output_dims, fc_dropout_p, *args, **kwargs):
        ...
        # fc_output_dims is the list of sizes of output channels fc_block
        self.critic_fc_layers = nn_layers(input_channels = gmt_output_dims, block = fc_block, output_channels = fc_output_dims, activation_func = 'relu', dropout_p = fc_dropout_p, weight_initialisation = "default")
