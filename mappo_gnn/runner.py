...
class Runner(object):
    ...
    def __init__(self, env, model):
        ...
        self.state = np.repeat(self.state, NUM_AGENTS, axis=1)
        self.cent_state = np.concatenate((self.obs, self.state), axis=-1)
        self.dones = [False for _ in range(N_ENVS)]
        self.actor_hidden_state_c = np.zeros((N_ENVS * NUM_AGENTS, ACTOR_LAYER2))
        self.actor_hidden_state_h = np.zeros((N_ENVS * NUM_AGENTS, ACTOR_LAYER2))
        self.critic_hidden_state_c = np.zeros((N_ENVS * NUM_AGENTS, CRITIC_LAYER2))
        self.critic_hidden_state_h = np.zeros((N_ENVS * NUM_AGENTS, CRITIC_LAYER2))
    ...
    def run(self):
        ...
        for _ in range(N_STEPS):
            ...
            if len(true_index) != 0:
                ...
                self.actor_hidden_state_c = np.reshape(self.actor_hidden_state_c, (-1, NUM_AGENTS, ACTOR_LAYER2))
                self.actor_hidden_state_h = np.reshape(self.actor_hidden_state_h, (-1, NUM_AGENTS, ACTOR_LAYER2))
                self.critic_hidden_state_c = np.reshape(self.critic_hidden_state_c, (-1, NUM_AGENTS, CRITIC_LAYER2))
                self.critic_hidden_state_h = np.reshape(self.critic_hidden_state_h, (-1, NUM_AGENTS, CRITIC_LAYER2))
        ...
        mb_rewards = np.repeat(mb_rewards, NUM_AGENTS, axis=-1)
        ...
        mb_actor_hidden_state_c = np.reshape(mb_actor_hidden_state_c, (N_STEPS, -1, NUM_AGENTS, ACTOR_LAYER2))
        mb_actor_hidden_state_h = np.reshape(mb_actor_hidden_state_h, (N_STEPS, -1, NUM_AGENTS, ACTOR_LAYER2))
        mb_critic_hidden_state_c = np.reshape(mb_critic_hidden_state_c, (N_STEPS, -1, NUM_AGENTS, CRITIC_LAYER2))
        mb_critic_hidden_state_h = np.reshape(mb_critic_hidden_state_h, (N_STEPS, -1, NUM_AGENTS, CRITIC_LAYER2))
        ...
        for t in reversed(range(N_STEPS)):
            if t == N_STEPS - 1:
                ...
                next_nonterminal = np.repeat(next_nonterminal, NUM_AGENTS, axis=1)
