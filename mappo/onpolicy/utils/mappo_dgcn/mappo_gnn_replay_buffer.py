...
class mappo_gnn_replay_buffer:
    ...
    def __init__(self, num_agents, batch_size):
        ...
        self.actor_action_log_probs_log_list = []
        ...
        for actor_index in range(self.num_agents):
            ...
            self.actor_action_log_list.append([])
            self.actor_action_log_probs_log_list.append([])
            self.critic_state_value_log_list.append([])
            self.rewards_log.append([])
            self.terminal_log.append([])
    
    def log(self, actor_state, critic_state, critic_state_value, action, action_log_probs, rewards, is_done):
        ...
        for actor_index in range(self.num_agents):
            ...
            # log actor_state, action and their log probabiities for each actor, critic_state_value, rewards and terminal 
            self.actor_state_log_list[actor_index].append(actor_state[actor_index])
            self.actor_action_log_list[actor_index].append(action[actor_index])
            self.actor_action_log_probs_log_list[actor_index].append(action_log_probs[actor_index])
    ...
    def sample_log(self, batch_size):
        ...
        return np.array(self.actor_state_log_list), np.array(self.actor_action_log_list), np.array(self.actor_action_log_probs_log_list), self.critic_state_log_list, np.array(self.critic_state_value_log_list), \
               np.array(self.rewards_log), np.array(self.terminal_log), batches